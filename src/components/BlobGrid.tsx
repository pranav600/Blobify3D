...
export default function BlobGrid() {
  return (
    <section
      id="blobgrid"
      className="relative w-full max-w-[95%] mx-auto mt-12 pb-8 scroll-mt-24">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {mockBlobs.map((blob, index) => (
          <motion.div
            ...
            <div className="absolute top-4 left-4 right-4 flex justify-between items-center opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity">
            ...
            <div className="absolute bottom-4 right-4 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity z-10">
              <a
                href={blob.image}
                download={`${blob.title.replace(/\s+/g, "_")}.png`}
                onClick={(e) => e.stopPropagation()}
                className="bg-white/80 backdrop-blur-md w-10 h-10 rounded-full text-black hover:bg-white hover:scale-110 transition-all border border-white shadow-md flex items-center justify-center cursor-pointer"
      ...
      <motion.div
        ...
        <div className="bg-black text-white px-5 py-4 sm:px-8 sm:py-5 rounded-[24px] shadow-2xl flex flex-col sm:flex-row items-center gap-4 sm:gap-6 border border-white/20 text-center sm:text-left">
          ...
          <button className="bg-blob-cyan text-black px-6 py-2.5 rounded-full font-bold hover:scale-105 transition-transform whitespace-nowrap w-full sm:w-auto cursor-pointer">
            Sign Up Now
          </button>
        </div>
      </motion.div>
